...
function isExpired(tokenOrCode) {
  ...
  //support for built-in accesstoken column
  if(tokenOrCode.hasOwnProperty('ttl') && tokenOrCode.hasOwnProperty('created')) {
    var elapsedSeconds = (now - tokenOrCode.created.getTime()) / 1000;
    return !(elapsedSeconds < tokenOrCode.ttl); 
  }
